...
const columns = [
  ...
  },
  {
    ...
    dataIndex: 'status',
    render: (text, record) => (
      <span className='bg-[#ffe0c3] text-[#ffb671] rounded px-2 py-1'>
        {record.status}
      </span>
    ),
  },
...
const Collection = () => {
  ...
  const start = () => {
    setLoading(true);
    // Simulate an AJAX request to mark rows as complete
    setTimeout(() => {
      // Update the status of selected rows to 'Complete'
      ...
      // Update the data with the new statuses
  ...
  const enableMarkAsComplete = hasSelected;
      ...
      <div className="grid grid-cols-5 grid-rows-7 gap-2 bg-[#f8f7fa]">
        <div className="col-span-4 row-span-4 bg-[#ffff] shadow-[rgba(17,_17,_26,_0.1)_0px_0px_16px] rounded p-4 h-fit">
          <div className="flex justify-between items-center mb-3">
            <div className="text-xl font-medium text-[#5c586b]">Collection/Delivery Status</div>
            <div>
              <button className="font-normal rounded rounded-xl bg-indigo-600 gap-3 px-3 py-2 text-white text-sm"   onClick={start}
                disabled={!hasSelected}>Mark as Complete</button>
            </div>
          </div>
          <Table 
           columns={columns}
